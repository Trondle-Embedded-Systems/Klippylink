...
/* Simple CRC-32/ISO-HDLC table-driven implementation. */
static uint32_t crc32_update(uint32_t crc, const uint8_t *data, size_t len)
{
    static const uint32_t table[256] = {
        0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA,
        0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
        /* abbreviated — full table follows in ROM via esp_rom_crc32_le */
    };
    /* Use the ROM CRC helper provided by ESP-IDF */
...
esp_err_t ota_end(void)
{
    ...
    /* Verify CRC32 of the written data */
    uint32_t actual_crc = 0;
    const uint8_t *flash = (const uint8_t *)s_part->address;
    /* Read-back check via esp_partition_read */
    uint8_t *verify_buf = malloc(256);
    if (verify_buf) {
        uint32_t offset = 0;
        while (offset < s_received_bytes) {
            size_t chunk = ((s_received_bytes - offset) < 256)
                           ? (s_received_bytes - offset) : 256;
            esp_partition_read(s_part, offset, verify_buf, chunk);
            actual_crc = esp_rom_crc32_le(actual_crc, verify_buf, chunk);
            offset += chunk;
        }
        free(verify_buf);
    }

    /* Note: if no crc32 was set (== 0) skip verification */
    if (s_expected_crc != 0 && actual_crc != s_expected_crc) {
